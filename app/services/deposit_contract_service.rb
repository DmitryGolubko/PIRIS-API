class DepositContractService
  def self.create(deposit_contract_params)
    ...
    make_transactions(deposit_contract, deposit)
    deposit_contract
  end
  ...
  def self.make_transactions(deposit_contract, deposit)
    ...
    Transaction.create!(sum: deposit_contract.sum, destination_account: cashbox)
    Transaction.create!(sum: deposit_contract.sum, source_account: cashbox,
                        destination_account: deposit_contract.current_account)
    Transaction.create!(sum: deposit_contract.sum, source_account: deposit_contract.current_account,
